...
class LTM():
    '''This class allows for generation of LTM objects used to store memories from previous chats. Once initialized, it takes care of the work of storing and retrieving previous chat comments from the user
    '''
    def __init__(self,
                 collection, 
                 verbose = True, 
    ...
    def create_vector_db_if_missing(self):
        try:
            ...
            if self.verbose: print(f"created self.collection: {self.collection}")
        except Exception as e:
            if self.verbose: 
                vectors_count = self.qdrant.get_collection(self.collection).vectors_count
                if self.verbose: print(f"self.collection: {self.collection} already exists with {vectors_count} vectors, not creating")
    ...
    def format_results_from_qdrant(self, results):
        ...
        for count, result in enumerate(results,1):
            if self.verbose: print(f"({count}/{len(results)}): vdb result score: {result.score}: {result.payload['comment']}\n")
    ...
    def store_and_recall(self, comment):
        ...
        if self.verbose: print(f"len of this object:{len(self)}")
